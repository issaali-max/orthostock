...
// ═══ صفقة سهم محذوف/مكرر (رمز غير نشط) يجب ألا تستنزف النقد ═══
{
  ...
  const st = pstats2(d);
  // كان النقد يتجاهل صفقات أي ورقة معطّلة، فكان إخفاء سهم يحرّك النقد بلا تدفق حقيقي.
  // الآن `isActive` على الصفقة نفسها هو ما يحسم: الدمج يُعيد إسناد الدفعات للورقة
  // المُبقاة، والحذف يُقاعد صفقاتها صراحةً — فلا تبقى دفعة يتيمة إلا في بيانات
  // مُركَّبة يدوياً كهذه. وطالما الدفعة حيّة فقد دُفع ثمنها فعلاً ويجب أن تُطرح.
  nq(st.cash, -32720, 'النقد: إيداع 20700 − شراء 20700 − الدفعة اليتيمة الحيّة 32720');
  nq(st.holdingsValue, 34000, 'الأسهم: 80×425 (UNH مرة واحدة)');

  // الحماية الحقيقية: تقاعد الدفعة اليتيمة يزيل أثرها النقدي — وهذا ما يفعله الحذف.
  const retired = { ...d, tradeLots: d.tradeLots.map((l) => (l.id === 'L2' ? { ...l, isActive: false } : l)) };
  nq(pstats2(retired).cash, 0, 'النقد: تقاعد الدفعة اليتيمة يُعيده إلى صفر');
  nq(pstats2(retired).holdingsValue, 34000, 'الأسهم: لا تتأثر');
