...
import { financialPosition, receivables, portfolioStats } from '../../lib/engine.js';
...
export default function FinancialPanel({ app }) {
  ...
  return (
    <div className="rise" style={{ marginBottom: 14 }}>
          ...
          <div style={{ display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ flex: 1, minWidth: 180, display: 'grid', gap: 7 }}>
              {slices.slice().sort((a, b) => b.value - a.value).map((s) => (
                <button key={s.key} onClick={() => setDrill(s.key === 'owed' ? 'personal' : s.key)}
      ...
      <PersonalModal open={drill === 'personal'} onClose={() => setDrill(null)} people={data[TABLES.externalDebts] || []} t={t} />
    </div>
  );
}
...
function PersonalModal({ open, onClose, people, t }) {
          ...
          {rows.map((p) => {
            ...
            return (
              <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 10, border: `1px solid ${C.border}`, borderRadius: 11, padding: '10px 12px' }}>
                ...
                <div style={{ fontWeight: 800, color: theyOwe ? C.warning : C.danger }}>{ccy(Math.abs(p.bal), code)}</div>
              </div>
            );
          })}
        </div>
