    ...
    <Modal open title={`🛒 ${t('purchasePlan') || 'قائمة المشتريات'}`} onClose={onClose} width={660}
      ...
      {counts.total === 0 ? (
        ...
        <div style={{ display: 'grid', gap: 14 }}>
          ...
          {buckets.map((b) => (
            <div key={b.sid || '__none'} style={{ border: `1.5px solid ${C.border}`, borderRadius: 14, padding: 10, background: '#fff' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ fontSize: 14, fontWeight: 900, color: C.text }}>🏭 {b.name}</div>
                  <div style={{ fontSize: 10.5, color: C.textMuted }}>{b.count} {t('itemsToReorder') || 'مادة'} · {t('estCost') || 'تقدير'}: {money(b.est, cur)}</div>
                </div>
                <Btn size="sm" variant="light" onClick={() => shareOrder(b)}>📤</Btn>
                <Btn size="sm" onClick={() => printList(b)}>🖨️</Btn>
              </div>
                          ...
                          {g.needed.map((it) => (
                            <div key={it.v.id} style={{ background: '#fff', borderRadius: 9, padding: '7px 10px' }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 5 }}>
                                ...
                                <div style={{ flex: 1 }}>
                                  <Select value={b.sid === '' ? '__none' : b.sid} onChange={(sel) => moveToSupplier(it.v, sel)} options={supOptions} />
                                </div>
                              </div>
                            </div>
                          ))}
